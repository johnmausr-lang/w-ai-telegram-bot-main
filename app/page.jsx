...
export default function NeonGlowAI() {
  ...
  const generatePhoto = async () => {
    ...
    setGenerating(true);
    setMessages(prev => [...prev, { role: "assistant", content: "Делаю горячее фото... 15–25 сек" }]);
    ...
    try {
      ...
      setMessages(prev =>
        prev.filter(m => m.content !== "Делаю горячее фото... 15–25 сек")
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-purple-950 via-pink-900 to-black text-white relative">
      ...
      {/* Верхняя панель */}
      {step !== "welcome" && (
        <div className="fixed top-0 left-0 right-0 z-40 bg-black/70 backdrop-blur-lg border-b border-white/10 p-4 flex justify-between">
