...
export default function NeonGlowAI() {
  ...
  const speak = async (text) => {
    ...
    try {
      const voice = personality.gender === "Женщина" ? "nova" : personality.gender === "Мужчина" ? "echo" : "alloy";
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const fallback = personality.mode === "flirt" ? "Ммм... я стесняюсь ❤️" : "Я рядом!";
  ...
  // Генерация фото от AI
  const generatePhoto = async () => {
    if (generatingPhoto) return;
    setGeneratingPhoto(true);
    try {
      const prompt = `красивая ${personality.gender === "Женщина" ? "девушка" : "парень"}, ${Object.values(personality.testAnswers).join(", ")}, неон, киберпанк, романтично, высокое качество`;
      ...
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      setMessages((m) => [...m, { role: "assistant", content: "Вот моё новое фото ❤️", image: url }]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-black to-pink-900 text-white overflow-hidden relative">
      ...
      <AnimatePresence mode="wait">
        {/* WELCOME */}
        {step === "welcome" && (
          <motion.div key="welcome" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center min-h-screen p-6">
        ...
        {/* SETUP + ТЕСТ + АВАТАР */}
        {step === "setup" && (
          <motion.div key="setup" initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} className="min-h-screen p-6 flex flex-col">
            <h2 className="text-5xl font-bold text-center mb-10">Создай своего AI</h2>
            ...
            {personality.orientation && !personality.mode && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-4xl mx-auto">
                <motion.div whileHover={{ scale: 1.05 }} onClick={(e) => { e.preventDefault(); setPersonality(p => ({ ...p, mode: "friend" })); }}
                  className="p-16 rounded-3xl backdrop-blur-xl border-4 border-cyan-400 bg-cyan-900/30 cursor-pointer z-50">
                ...
                <motion.div whileHover={{ scale: 1.05 }} onClick={(e) => { e.preventDefault(); setPersonality(p => ({ ...p, mode: "flirt" })); }}
                  className="p-16 rounded-3xl backdrop-blur-xl border-4 border-pink-400 bg-pink-900/40 cursor-pointer z-50">
            ...
            {personality.mode && !personality.testDone && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10 max-w-2xl mx-auto">
                ...
                {[
                  { q: "Характер?", a: ["Нежный", "Смелый", "Шутник", "Таинственный"] },
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {item.a.map((ans) => (
                        <button key={ans} onClick={(e) => { e.preventDefault(); setPersonality(p => ({ ...p, testAnswers: { ...p.testAnswers, [i]: ans } })); }}
                          className="py-5 rounded-xl bg-white/10 hover:bg-pink-500/30 transition border border-white/20">
                          {ans}
                        </button>
                      ))}
                    </div>
        ...
        {/* ЧАТ */}
        {step === "chat" && (
          <motion.div key="chat" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col min-h-screen pb-32">
            <div className="p-6 text-center">
              <h2 className="text-4xl font-bold">Твой AI здесь</h2>
              ...
              {messages.map((m, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className={`max-w-md px-6 py-4 rounded-3xl backdrop-blur-xl border-2 ${m.role === "user" ? "bg-white/20 border-white/30" : "bg-pink-900/40 border-pink-400/50"}`}>
                    {m.image ? <img src={m.image} alt="AI photo" className="rounded-2xl max-w-full" /> : m.content}
                  </div>
                </motion.div>
              ))}
              {loading && <div className="text-center"><span className="animate-pulse text-2xl">Пишет...</span></div>}
            </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black/90 to-transparent">
              <div className="max-w-4xl mx-auto flex gap-4">
                <input
                  ...
                  className="flex-1 px-8 py-6 rounded-full bg-white/10 backdrop-blur-xl border-2 border-white/20 text-xl focus:outline-none focus:border-pink-400"
                />
